...
> // 运行（先接收端，后发送端）
> receiver.exe 0.0.0.0 9000 recv.bin 64
> sender.exe 127.0.0.1 9000 test.bin 64 0.0
> ```
...
- `sack_mask` 使用 32 位位图，表示在 `ack` 之后的 32 个分片的接收情况
...
为了便于实验分析，在发送端对 DATA 分组引入可配置的“模拟丢包率”，用于观察不同丢包环境下协议和拥塞控制的行为。
